 ...
 */
std::vector<Eigen::MatrixXd> Robot::CalculateTransformations(const Eigen::VectorXd& config) {
...
Eigen::MatrixXd Robot::GetJacobian(const Eigen::VectorXd& config) {
    ...
    for (int i = 1; i <= Robot_DHParam.size(); i++) {
        ...
        Eigen::VectorXd Ji(6);
        Ji << Ji_linear[0], Ji_linear[1], Ji_linear[2], Ji_angular[0], Ji_angular[1], Ji_angular[2];
...
/**
      ...
      */
const std::vector<std::pair<double, double>>& Robot::getCirclePoints() const {
